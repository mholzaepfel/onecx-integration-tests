...
export async function importSlots(baseDir: string, endpointBase: string) {
  ...
  for (const file of files) {
    ...
    // New onecx-data format: filename is product and payload contains [{ appId, slots: [...] }]
    if (
      ...
      for (const entry of parsedData) {
        ...
        logger.info(
          'PROCESSING_FILE',
          `${file} - Product: ${product}, App: ${entry.appId}, Slots: ${entry.slots.length}`
        )

        try {
          const response = await axios.put(endpoint, entry.slots, {
            headers: { 'Content-Type': 'application/json' },
            validateStatus: () => true,
          })
          logger.status(
            'UPLOAD_SUCCESS',
            response.status,
            `Slots (${entry.slots.length}) for app ${entry.appId} and product ${product}`
          )
        } catch (err) {
          logger.error('UPLOAD_ERROR', `Slots for app ${entry.appId} and product ${product}`, err)
        }
      }
    ...
    // Legacy format: filename is {product}_{appId}_{slotName}.json and payload is a single slot object
...
export async function importMicrofrontends(baseDir: string, endpointBase: string, port: number) {
  ...
  for (const file of files) {
    ...
    // Transform URLs if they don't start with http. Use app id from filename as Docker host.
